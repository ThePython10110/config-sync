...
destroynotify(XEvent *e)
{
	...
	if (!(c = wintoclient(ev->window)) && (c = wintosystrayicon(ev->window))) {
		removesystrayicon(c);
		resizebarwin(selmon);
		updatesystray();
	}
...
sendmon(Client *c, Monitor *m)
{
	...
	attachstack(c);
	arrange(m);
	if (hadfocus && selmon->wfsymbol[0] == WFACTIVE) {
		focus(c);
		restack(m);
	} else
		focus(NULL);
...
updatesystray(void)
{
	...
	XFillRectangle(dpy, systray->win, XCreateGC(dpy, systray->win, 0, NULL), 0, 0, w, bh);
	XSync(dpy, False);
}
